#!/usr/bin/env python3
"""
Replace the seed command with a FAST version.

The old command did ~864 update_or_create calls = ~1,700 round-trips to the
remote Supabase DB (minutes over a high-latency link). This version builds all
rows in memory and writes them with delete + bulk_create inside one transaction
= ~3 round-trips total (seconds). Idempotent: it wipes only its own seed rows
(source="Seed data (sample)") and re-inserts, never touching real data.
...
def main():
    ...
    target = "marketprices/management/commands/seed_market_prices.py"
    if not os.path.exists(target):
        sys.exit(f"ERROR: missing {target}")
    with open(target, "w", encoding="utf-8") as f:
        f.write(SEED)
    print(f"  [edit] {target} -> fast bulk version")
    print("\nNow run:  python manage.py seed_market_prices  (should finish in seconds)")
